...
def generate_instance_crossover_interpolation(X_train,X_explain, y_explain,indep,dep,blackbox_model, random_state=None, debug=False):
        ...
        for count, case in cases_normalize.iterrows():
            ...
            top_fourty_df = pd.concat(top_fourty_list, axis=0)
            ...
            for _, row in target_details_df.iterrows():
                ...
                neigh_chunks.append(filterd_class_set)

            final_neighbours_similarity_df = pd.concat(neigh_chunks, axis=0)
            ...
            for num in range(1000, 2000):
                ...
                new_ins.name = num
                new_con_list.append(new_ins)
            new_con_df = pd.concat(new_con_list, ignore_index=True)
            ...
            # get the global model predictions of the generated instances and the instances in the neighbourhood
            predict_dataset = pd.concat([train_set_neigh, new_con_df], ignore_index=True)
